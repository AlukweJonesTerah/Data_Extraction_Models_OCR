...
copies the file flat into \\192.168.1.11\d\Fullset samples
(no subfolders, filename prefixed with batch+subfolder to avoid collisions).
...
# ── Paths ─────────────────────────────────────────────────────────────────────
MODEL_PATH   = r"\Users\Administrator\Desktop\florence2-nssf-ocr-final"
SOURCE_ROOT  = r"\\192.168.1.11\phase_v\BCERTS\RAW"
DEST_FOLDER  = r"\\192.168.1.11\d\Fullset samples2"
...
def classify_page(pil_image, processor, model, device="cpu"):
...
# ── Main scan ─────────────────────────────────────────────────────────────────
def run_scan(processor, model, device="cpu"):
    ...
    for file_idx, (batch_name, img_folder, file_path) in enumerate(all_files):
        ...
        for page in range(n_pages):
            ...
            try:
                doc_type     = classify_page(frame, processor, model, device)
        ...
        # Copy only fingerprint form files, flat into DEST_FOLDER
        if file_is_fingerprint:
            fp_files += 1
            # Prefix with batch+subfolder to avoid filename collisions
            flat_name = f"{batch_name}_{img_folder}_{fname}"
            dest_path = os.path.join(DEST_FOLDER, flat_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "--list-only":
        ...
        print("Model ready\n")
        run_scan(processor, model, DEVICE)